...
function SkillChip({ skill, delay, accentColor }: { skill: string; delay: number; accentColor: string }) {
  return (
    <motion.span
      className={`inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-medium rounded-lg bg-slate-100/80 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-slate-700/40 hover:border-current ${hoverColorMap[accentColor] || ''} transition-all duration-300 cursor-default group`}
